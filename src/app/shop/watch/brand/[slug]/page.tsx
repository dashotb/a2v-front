...
const Shop: NextPage<any> = ({ params }: { params: { slug: string } }) => {
    const pageId = params.slug

    //permet de savoir si c'est une categorie ou un numéro de page
    function fits(x: any, y: any) {
    ...
    if (fits(pageId, 0)) {
        //fetch articles + filtres
        const [products, setProducts] = useState<any[]>([]);
        const [filteredProducts, setFilteredProducts] = useState<any[]>([])
        const getProducts = async () => {
            const response = await fetch(`/api/products/watch/brand`).then((response) => response.json());
        ...
        return (
            <main className="container">
                <div className="title_page">
                    <a className="title_page_1">Montres</a>
                </div>
        ...
        const [filteredProducts, setFilteredProducts] = useState<any[]>([])
        const getProducts = async () => {
            const response = await fetch(`/api/products/watch/brand/${pageId}`).then((response) => response.json());
            setProducts(response);
            setFilteredProducts(response)
            console.log(response)
        }
        ...
        //permet de réécrire le titre 
        const s = pageId.replace(/([A-Z])/g, ' $1').trim()
        return (
            <main className="container">
                <div className="title_page">
