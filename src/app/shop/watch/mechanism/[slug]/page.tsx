...
const Shop: NextPage<any> = ({ params }: { params: { slug: string } }) => {
    ...
    if (fits(pageId, 0)) {
        ...
        const getProducts = async () => {
            const response = await fetch(`/api/products/watch/mechanism`).then((response) => response.json());
        ...
        return (
            <main className="container">
                <div className="title_page">
                    <a className="title_page_1">Mechanisme</a>
                </div>
        ...
        const [filteredProducts, setFilteredProducts] = useState<any[]>([])
        const getProducts = async () => {
            const response = await fetch(`/api/products/watch/mechanism/${pageId}`).then((response) => response.json());
            setProducts(response);
            setFilteredProducts(response)
            console.log(response)
        }
