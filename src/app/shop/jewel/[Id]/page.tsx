...
const Shop: NextPage<any> = ({ params }: { params: { Id: string } }) => {
    ...
    const getProducts = async () => {
        const response = await fetch(`/api/products/jewel`).then((response) => response.json());
    ...
    if (end > 0) {
        var page_nav = <div className="page_nav">
            ...
            {list.map(list => (
                <a><Link href="/shop/jewels/[id]" as={`/shop/jewels/${list}`}>{list}</Link></a>

            ))}
            <Link href="/shop/jewels/1">
    ...
    if (end > 9) {
        ...
        var page_nav = <div className="page_nav">
            <Image src={LeftArrow} alt="" width={15} height={15} />
            {rest.map(rest => (
                <a><Link href="/shop/jewels/[id]" as={`/shop/${rest}`}>{rest}</Link></a>

            ))} <a>...</a>
            <a><Link href="/shop/jewels/[id]" as={`/shop/${end}`}>{end}</Link></a>
            <Image src={RightArrow} alt="" width={15} height={15} />
