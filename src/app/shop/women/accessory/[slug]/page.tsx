...
const Shop: NextPage<any> = ({ params }: { params: { slug: string } }) => {
    ...
    if (fits(pageId, 0)) {
        ...
        const getProducts = async () => {
            const response = await fetch(`/api/products/gender/women/accessory`).then((response) => response.json());
        ...
        const [filteredProducts, setFilteredProducts] = useState<any[]>([])
        const getProducts = async () => {
            const response = await fetch(`/api/products/gender/women/accessory/${pageId}`).then((response) => response.json());
            setProducts(response);
            setFilteredProducts(response)
            console.log(response)
        }
