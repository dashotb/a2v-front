...
const Shop: NextPage<any> = ({ params }: { params: { Id: string } }) => {
    ...
    const getProducts = async () => {
        const response = await fetch(`/api/products/women`).then((response) => response.json());
    ...
    if (end > 0) {
        var page_nav = <div className="page_nav">
            ...
            {list.map(list => (
                <a><Link href="/shop/women/[id]" as={`/shop/${list}`}>{list}</Link></a>

            ))}
            <Link href="/shop/women/1">
    ...
    if (end > 9) {
        ...
        var page_nav = <div className="page_nav">
            <Image src={LeftArrow} alt="" width={15} height={15} />
            {rest.map(rest => (
                <a><Link href="/shop/wommen/[id]" as={`/shop/${rest}`}>{rest}</Link></a>

            ))} <a>...</a>
            <a><Link href="/shop/women/[id]" as={`/shop/${end}`}>{end}</Link></a>
            <Image src={RightArrow} alt="" width={15} height={15} />
                    ...
                    <div className="sort">
                        <div className="sort_by">
                            ...
                            <select name="cat" id="original" className="postform" onChange={e => Sort(e.target.value)}>
                                ...
                                <option className="level-0" value="date">Date</option>
